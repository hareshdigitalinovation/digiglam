<?php
namespace RoslynCore\CPT\Shortcodes\Banner;
...
class Banner implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Banner', 'roslyn-core' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						...
						array(
							...
							'heading'    => esc_html__( 'Image Overlay Color', 'roslyn-core' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'hover_behavior',
							'heading'     => esc_html__( 'Hover Behavior', 'roslyn-core' ),
							'value'       => array(
								esc_html__( 'Visible on Hover', 'roslyn-core' )   => 'eltdf-visible-on-hover',
								esc_html__( 'Visible on Default', 'roslyn-core' ) => 'eltdf-visible-on-default',
								esc_html__( 'Disabled', 'roslyn-core' )           => 'eltdf-disabled'
							),
							'save_always' => true
						),
						array(
							...
							'heading'     => esc_html__( 'Info Position', 'roslyn-core' ),
							'value'       => array(
								esc_html__( 'Default', 'roslyn-core' )  => 'default',
								esc_html__( 'Centered', 'roslyn-core' ) => 'centered'
							),
							'save_always' => true
						),
						array(
							...
							'heading'     => esc_html__( 'Info Content Padding', 'roslyn-core' ),
							'description' => esc_html__( 'Please insert padding in format top right bottom left', 'roslyn-core' )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'subtitle',
							'heading'    => esc_html__( 'Subtitle', 'roslyn-core' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'subtitle_tag',
							'heading'     => esc_html__( 'Subtitle Tag', 'roslyn-core' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Subtitle Color', 'roslyn-core' ),
							'dependency' => array( 'element' => 'subtitle', 'not_empty' => true )
						),
						array(
							'type'       => 'textfield',
						...
						array(
							...
							'heading'    => esc_html__( 'Title Top Margin (px)', 'roslyn-core' ),
							'dependency' => array( 'element' => 'title', 'not_empty' => true )
						),
						array(
							'type'       => 'textfield',
						...
						array(
							...
							'heading'    => esc_html__( 'Link Text', 'roslyn-core' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						),
						array(
							'type'       => 'colorpicker',
							'param_name' => 'link_color',
							'heading'    => esc_html__( 'Link Text Color', 'roslyn-core' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'link_top_margin',
							'heading'    => esc_html__( 'Link Text Top Margin (px)', 'roslyn-core' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						)
					)
				)
	...
	public function render( $atts, $content = null ) {
		...
		$html = roslyn_core_get_shortcode_module_template_part( 'templates/banner', 'banner', '', $params );
	...
	private function getTitleStyles( $params ) {
		...
		if ( ! empty( $params['title_top_margin'] ) ) {
			$styles[] = 'margin-top: ' . roslyn_elated_filter_px( $params['title_top_margin'] ) . 'px';
		}
	...
	private function getLinkStyles( $params ) {
		...
		if ( ! empty( $params['link_top_margin'] ) ) {
			$styles[] = 'margin-top: ' . roslyn_elated_filter_px( $params['link_top_margin'] ) . 'px';
		}
