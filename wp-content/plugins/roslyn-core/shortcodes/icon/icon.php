<?php
namespace RoslynCore\CPT\Shortcodes\Icon;
...
class Icon implements Lib\ShortcodeInterface {
    ...
    public function vcMap() {
	    if(function_exists('vc_map')) {
		    vc_map(
		    	array(
				    'name'                      => esc_html__( 'Icon', 'roslyn-core' ),
				    'base'                      => $this->base,
				    ...
				    'params'                    => array_merge(
					    ...
					    array(
						    array(
							    ...
							    'value'      => array(
								    esc_html__( 'Tiny', 'roslyn-core' )   => 'eltdf-icon-tiny',
								    esc_html__( 'Small', 'roslyn-core' )  => 'eltdf-icon-small',
								    esc_html__( 'Medium', 'roslyn-core' ) => 'eltdf-icon-medium',
								    esc_html__( 'Large', 'roslyn-core' )  => 'eltdf-icon-large',
								    esc_html__( 'Huge', 'roslyn-core' )   => 'eltdf-icon-huge'
							    )
						    ),
						    array(
							    'type'       => 'textfield',
							    'param_name' => 'custom_size',
							    'heading'    => esc_html__( 'Custom Size (px)', 'roslyn-core' )
						    ),
						    array(
							    'type'       => 'dropdown',
							    'param_name' => 'type',
							    ...
							    'value'      => array(
								    esc_html__( 'Normal', 'roslyn-core' ) => 'eltdf-normal',
								    esc_html__( 'Circle', 'roslyn-core' ) => 'eltdf-circle',
								    esc_html__( 'Square', 'roslyn-core' ) => 'eltdf-square'
							    ),
							    'save_always' => true
						    ),
						    array(
							    ...
							    'heading'     => esc_html__( 'Border Radius', 'roslyn-core' ),
							    'description' => esc_html__( 'Please insert border radius(Rounded corners) in px. For example: 4 ', 'roslyn-core' ),
							    'dependency'  => array( 'element' => 'type', 'value' => array( 'eltdf-square' ) )
						    ),
						    array(
							    'type'       => 'textfield',
							    'param_name' => 'shape_size',
							    'heading'    => esc_html__( 'Shape Size (px)', 'roslyn-core' ),
						    ...
						    array(
							    ...
							    'heading'    => esc_html__( 'Border Width', 'roslyn-core' ),
							    'dependency' => array( 'element' => 'type', 'value'   => array( 'eltdf-circle', 'eltdf-square' ) )
						    ),
						    array(
							    'type'       => 'colorpicker',
						    ...
						    array(
							    ...
							    'heading'    => esc_html__( 'Hover Icon Color', 'roslyn-core' )
						    ),
						    array(
							    'type'       => 'colorpicker',
						    ...
						    array(
							    ...
							    'heading'    => esc_html__( 'Icon Animation', 'roslyn-core' ),
							    'value'      => array_flip( roslyn_elated_get_yes_no_select_array( false ) )
						    ),
						    array(
							    'type'       => 'textfield',
							    'param_name' => 'icon_animation_delay',
							    'heading'    => esc_html__( 'Icon Animation Delay (ms)', 'roslyn-core' ),
						    ...
						    array(
							    ...
							    'heading'     => esc_html__( 'Use Link as Anchor', 'roslyn-core' ),
							    'value'       => array( 'Use this icon as Anchor?' => 'yes' ),
							    'description' => esc_html__( 'Check this box to use icon as anchor link (eg. #contact)', 'roslyn-core' ),
							    'dependency'  => Array( 'element' => 'link', 'not_empty' => true )
						    ),
						    array(
							    'type'       => 'dropdown',
							    'param_name' => 'target',
							    'heading'    => esc_html__( 'Target', 'roslyn-core' ),
	...
	public function render( $atts, $content = null ) {
		...
		$html = roslyn_core_get_shortcode_module_template_part( 'templates/icon', 'icon', '', $params );
	...
	private function generateIconStyles( $params ) {
		...
		if ( ( $params['type'] !== 'eltdf-normal' && ! empty( $params['shape_size'] ) ) || ( $params['type'] == 'eltdf-normal' ) ) {
			if ( ! empty( $params['custom_size'] ) ) {
				$iconStyles[] = 'font-size:' . roslyn_elated_filter_px( $params['custom_size'] ) . 'px';
			}
		}
	...
	private function generateIconHolderStyles( $params ) {
		...
		if ( $params['type'] !== 'eltdf-normal' ) {
			...
			if ( ! empty( $shapeSize ) ) {
				$iconHolderStyles[] = 'width: ' . roslyn_elated_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'height: ' . roslyn_elated_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'line-height: ' . roslyn_elated_filter_px( $shapeSize ) . 'px';
			}
			...
			if ( ! empty( $params['border_color'] ) && ( isset( $params['border_width'] ) && $params['border_width'] !== '' ) ) {
				...
				$iconHolderStyles[] = 'border-width: ' . roslyn_elated_filter_px( $params['border_width'] ) . 'px';
			...
			if ( $params['type'] == 'eltdf-square' ) {
				if ( isset( $params['border_radius'] ) && $params['border_radius'] !== '' ) {
					$iconHolderStyles[] = 'border-radius: ' . roslyn_elated_filter_px( $params['border_radius'] ) . 'px';
				}
			}
		}
