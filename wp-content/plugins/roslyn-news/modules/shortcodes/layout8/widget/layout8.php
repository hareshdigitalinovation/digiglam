...
class RoslynNewsClassWidgetLayout8 extends RoslynNewsPhpClassWidget {
	public function __construct() {
		parent::__construct(
			'eltdf_layout8_widget',
			esc_html__( 'Roslyn Layout 8 Widget', 'roslyn-news' ),
			array( 'description' => esc_html__( 'Display a layout 8', 'roslyn-news' ) )
		);
	...
	public function widget( $args, $instance ) {
		...
		echo '<div class="widget eltdf-news-widget eltdf-news-layout8-widget">';
			if ( ! empty( $instance['widget_title'] ) ) {
				echo wp_kses_post( $args['before_title'] ) . esc_html( $instance['widget_title'] ) . wp_kses_post( $args['after_title'] );
			}
			
			echo roslyn_elated_execute_shortcode( 'eltdf_layout8', $instance ); // XSS OK
		echo '</div>';
	}
}