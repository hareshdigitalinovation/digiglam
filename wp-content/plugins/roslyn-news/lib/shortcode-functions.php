...
if ( ! function_exists( 'roslyn_news_include_shortcodes_file' ) ) {
	/**
	 * Loades all shortcodes by going through all folders that are placed directly in shortcodes folder
	 */
	function roslyn_news_include_shortcodes_file() {
		foreach ( glob( ROSLYN_NEWS_SHORTCODES_PATH . '/*/load.php' ) as $shortcode_load ) {
			include_once $shortcode_load;
		}
		
		do_action( 'roslyn_news_action_include_shortcodes_file' );
	}
	
	add_action( 'init', 'roslyn_news_include_shortcodes_file', 6 ); // permission 6 is set to be before vc_before_init hook that has permission 9
}

if ( ! function_exists( 'roslyn_news_load_shortcodes' ) ) {
	function roslyn_news_load_shortcodes() {
		include_once ROSLYN_NEWS_LIB_PATH . '/shortcode-loader.php';
		
		RoslynNews\Lib\ShortcodeLoader::getInstance()->load();
	}
	
	add_action( 'init', 'roslyn_news_load_shortcodes', 7 ); // permission 7 is set to be before vc_before_init hook that has permission 9 and after roslyn_core_include_shortcodes_file hook
}

if ( ! function_exists( 'roslyn_news_add_admin_shortcodes_styles' ) ) {
	/**
	 * Function that includes shortcodes core styles for admin
	 */
	function roslyn_news_add_admin_shortcodes_styles() {
		//include shortcode styles for Visual Composer
		wp_enqueue_style( 'roslyn-news-vc-shortcodes', ROSLYN_NEWS_ASSETS_URL_PATH . '/css/admin/roslyn-vc-shortcodes.css' );
	}
	
	add_action( 'roslyn_elated_admin_scripts_init', 'roslyn_news_add_admin_shortcodes_styles' );
}

if ( ! function_exists( 'roslyn_news_add_admin_shortcodes_custom_styles' ) ) {
	/**
	 * Function that print custom vc shortcodes style
	 */
	function roslyn_news_add_admin_shortcodes_custom_styles() {
		$style                  = apply_filters( 'roslyn_news_filter_add_vc_shortcodes_custom_style', $style = '' );
		$shortcodes_icon_styles = array();
		$shortcode_icon_size    = 32;
		$shortcode_position     = 0;
		
		$shortcodes_icon_class_array = apply_filters( 'roslyn_news_filter_add_vc_shortcodes_custom_icon_class', $shortcodes_icon_class_array = array() );
		...
		if ( ! empty( $style ) ) {
			wp_add_inline_style( 'roslyn-news-vc-shortcodes', $style );
		}
	}
	
	add_action( 'roslyn_elated_admin_scripts_init', 'roslyn_news_add_admin_shortcodes_custom_styles' );
}

if ( ! function_exists( 'roslyn_news_get_query' ) ) {
	/**
	 ...
	 *
	 */
	function roslyn_news_get_query( $params, $return_query = true ) {
		...
		if ( $params['category_name'] !== '' ) {
			$query_array['category_name'] = str_replace( ' ', '', $params['category_name'] );
		}
...
if ( ! function_exists( 'roslyn_news_get_shortcode_params_names' ) ) {
	/**
	 ...
	 *
	 */
	function roslyn_news_get_shortcode_params_names( $params_array ) {
		$params_names = array();
...
if ( ! function_exists( 'roslyn_news_get_holder_data_params' ) ) {
	/**
	 ...
	 *
	 */
	function roslyn_news_get_holder_data_params( $params, $base ) {
		$data_string = '';
		
		$query_result = roslyn_news_get_query( $params, false );
...
if ( ! function_exists( 'roslyn_news_shortcodes_load_more' ) ) {
	function roslyn_news_shortcodes_load_more() {
		$params = array();
		...
		$layout       = str_replace( 'eltdf_', '', $params['layout'] );
		$layout       = str_replace( '_', ' ', $layout );
		$layout       = str_replace( ' ', '', ucwords( $layout ) );
		$layout_class = ucfirst( $layout );
		
		$a = '\RoslynNews\CPT\Shortcodes\\' . $layout_class . '\\' . $layout_class;
	...
	add_action( 'wp_ajax_nopriv_roslyn_news_shortcodes_load_more', 'roslyn_news_shortcodes_load_more' );
	add_action( 'wp_ajax_roslyn_news_shortcodes_load_more', 'roslyn_news_shortcodes_load_more' );
}

if ( ! function_exists( 'roslyn_news_shortcodes_filter' ) ) {
	function roslyn_news_shortcodes_filter() {
		$params = array();
		...
		$query_params = roslyn_news_get_query( $params, false );
	...
	add_action( 'wp_ajax_nopriv_roslyn_news_shortcodes_filter', 'roslyn_news_shortcodes_filter' );
	add_action( 'wp_ajax_roslyn_news_shortcodes_filter', 'roslyn_news_shortcodes_filter' );
}