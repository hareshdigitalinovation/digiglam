...
class IP2LocationRedirection
{
	...
	public function update_ip2location_database()
	{
		...
		try {
			...
			$ipv6 = ($ipv4_only) ? '' : 'IPV6';
			$zip_file = IP2LOCATION_DIR . 'database.zip';
			...
			if (filesize($zip_file) < 51200) {
				exit(json_encode([
					'status'  => 'ERROR',
					'message' => __(file_get_contents($zip_file), 'ip2location-redirection'),
				]));
			}

			// Unzip the package to working directory
			$result = unzip_file($zip_file, IP2LOCATION_DIR);
			...
			foreach ($files as $file) {
				if (strtoupper(substr($file, -4)) == '.BIN') {
					if (preg_match('/^(IP2LOCATION|IP-COUNTRY)/', $file)) {
						$bin_database = $file;
					}
				} else {
					wp_delete_file(IP2LOCATION_DIR . $file);
				}
			}
			...
			if (!$bin_database) {
				exit(json_encode([
					'status'  => 'ERROR',
					'message' => __('No database found.', 'ip2location-redirection'),
				]));
			}
				...
				<div class="ip2location-modal-content">
					...
					<form>
						<p>
							...
							<input type="text" id="setup_api" class="regular-text code" maxlength="64" style="width:100%;margin-top: 10px; margin-bottom: 4px;">
						</p>
						<p class="description">
							' . sprintf(__('Don\'t have an account yet? Sign up a %1$s free IP geolocation plan%2$s to obtain your API key.', 'ip2location-redirection'), '<a href="https://www.ip2location.io/sign-up#wordpress-wzdir" target="_blank">', '</a>') . '
						</p>
						<p id="api_status">&nbsp;</p>
					</form>
					<p style="text-align:right;margin-top:30px">
