...
$row_content_width = $anchor = $content_text_aligment = $simple_background_color = $simple_background_image = $disable_background_image = $parallax_background_image = $parallax_bg_speed = $parallax_bg_height = $row_btt_skin = '';
...
if ( $row_content_width !== 'grid' ) {
	...
	if (!empty($row_btt_skin)){
		$css_classes[] = ($row_btt_skin) == 'light' ? 'eltdf-row-btt-light' : '';
	}
	...
	if (!empty($row_btt_skin)){
		$grid_row_class[] = ($row_btt_skin) == 'light' ? 'eltdf-row-btt-light' : '';
	}
}
...
if($row_content_width === 'grid'){
	$eltdf_row_wrapper_start .= '<div class="eltdf-row-grid-section-wrapper '.esc_attr($grid_row_classes).'" '.esc_attr($grid_row_datas).' ' . roslyn_elated_get_inline_style( $eltdf_grid_row_styles ) . '><div class="eltdf-row-grid-section">';
...
$output .= '<div ' . implode( ' ', $wrapper_attributes ) . ' ' . roslyn_elated_get_inline_style( $eltdf_vc_row_styles ) . '>';
$output .= wpb_js_remove_wpautop( $content );
$output .= '</div>';
$output .= $eltdf_row_wrapper_end;
$output .= $after_output;

echo roslyn_elated_get_module_part( $output );
