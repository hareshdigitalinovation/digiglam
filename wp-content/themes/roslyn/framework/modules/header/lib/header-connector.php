...
namespace RoslynElated\Modules\Header\Lib;
/**
 * Class RoslynElatedHeaderConnector
 *
...
class RoslynElatedHeaderConnector {
	...
	public function connect( $config = array() ) {
		do_action( 'roslyn_elated_pre_header_connect' );
		...
		if ( ! empty( $config['affect_content'] ) ) {
			add_filter( 'roslyn_elated_content_elem_style_attr', array( $this, 'contentMarginFilter' ) );
		}
		
		if ( ! empty( $config['affect_title'] ) ) {
			add_filter( 'roslyn_elated_title_content_padding', array( $this, 'titlePaddingFilter' ) );
		}
		
		do_action( 'roslyn_elated_after_header_connect' );
	}
	 ...
	 *
	 * Hooks to roslyn_elated_title_content_padding filter
	 *
	 * @return int
	 */
