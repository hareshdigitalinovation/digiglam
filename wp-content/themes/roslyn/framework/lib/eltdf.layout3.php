...
/*
   Class: RoslynElatedMultipleImages
   A class that initializes Elated Multiple Images
*/

class RoslynElatedMultipleImages implements iRoslynElatedRender {
	private $name;
	private $label;
	private $description;
	...
	function __construct( $name, $label = "", $description = "" ) {
		...
		$roslyn_elated_Framework->eltdMetaBoxes->addOption( $this->name, "" );
	}
					...
					<div class="row">
						<div class="col-lg-12">
							...
							<div class="eltdf-gallery-uploader">
								<a class="eltdf-gallery-upload-btn btn btn-sm btn-primary" href="javascript:void(0)"><?php esc_html_e( 'Upload', 'roslyn' ); ?></a>
								<a class="eltdf-gallery-clear-btn btn btn-sm btn-default pull-right" href="javascript:void(0)"><?php esc_html_e( 'Remove All', 'roslyn' ); ?></a>
							</div>
						</div>
					</div>
				</div>
...
class RoslynElatedTwitterFramework implements iRoslynElatedRender {
	public function render( $factory ) {
		$twitterApi = RoslynTwitterApi::getInstance();
		$message    = '';
		...
		if ( ! empty( $_GET['oauth_token'] ) && ! empty( $_GET['oauth_verifier'] ) ) {
			...
			if ( $responseObj->status ) {
				$message = esc_html__( 'You have successfully connected with your Twitter account. If you have any issues fetching data from Twitter try reconnecting.', 'roslyn' );
			} else {
				$message = $responseObj->message;
			}
		}
		
		$buttonText = $twitterApi->hasUserConnected() ? esc_html__( 'Re-connect with Twitter', 'roslyn' ) : esc_html__( 'Connect with Twitter', 'roslyn' );
		?>
		<?php if ( $message !== '' ) { ?>
		...
		<div class="eltdf-page-form-section" id="eltdf_enable_social_share">
			<div class="eltdf-field-desc">
				<h4><?php esc_html_e( 'Connect with Twitter', 'roslyn' ); ?></h4>
				<p><?php esc_html_e( 'Connecting with Twitter will enable you to show your latest tweets on your site', 'roslyn' ); ?></p>
			</div>
			<div class="eltdf-section-content">
				<div class="container-fluid">
					<div class="row">
...
class RoslynElatedInstagramFramework implements iRoslynElatedRender {
	public function render( $factory ) {
		...
		if ( ! empty( $_GET['code'] ) && ! empty( $_GET['instagram'] ) ) {
			...
			$message = esc_html__( 'You have successfully connected with your Instagram Personal account.', 'roslyn' );
		}
		...
		if ( ! empty( $_GET['access_token'] ) && ! empty( $_GET['facebook'] ) ) {
			...
			$message = esc_html__( 'You have successfully connected with your Instagram Business account.', 'roslyn' );
		}
		...
		if ( ! empty( $_GET['disconnect'] ) ) {
			...
			$message = esc_html__( 'You have have been disconnected from all Instagram accounts.', 'roslyn' );
		...
		<div class="eltdf-page-form-section" id="eltdf_enable_social_share">
			<div class="eltdf-field-desc">
				<h4><?php esc_html_e( 'Connect with Instagram', 'roslyn' ); ?></h4>
				<p><?php esc_html_e( 'Connecting with Instagram will enable you to show your latest photos on your site', 'roslyn' ); ?></p>
			</div>
			<div class="eltdf-section-content">
				<div class="container-fluid">
					<?php
					...
					if ( $instagram_user_id ) { ?>
						<div class="row">
							<div class="col-lg-12">
								<p><?php echo esc_html__( 'You are currently connected to Instagram ID: ', 'roslyn' );
					...
					<div class="row">
						<?php if ( ! empty( $_GET['disconnect'] ) ) { ?>
							<div class="col-lg-4">
								<a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->reloadURL() ); ?>"><?php echo esc_html__( 'Reload Page', 'roslyn' ); ?></a>
							</div>
						<?php } else if ( empty( $connection_type ) ) { ?>
							<div class="col-lg-4">
								<a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->instagramRequestCode() ); ?>"><?php echo esc_html__( 'Connect with Instagram Personal account', 'roslyn' ); ?></a>
							</div>
<!--							<div class="col-lg-4">-->
<!--								<a class="btn btn-primary" href="--><?php //echo esc_url( $instagram_api->facebookRequestCode() ); ?><!--">--><?php //echo esc_html__( 'Connect with Instagram Business account', 'roslyn' ); ?><!--</a>-->
<!--							</div>-->
						<?php } else { ?>
							<div class="col-lg-4">
								<a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->disconnectURL() ); ?>"><?php echo esc_html__( 'Disconnect Instagram account', 'roslyn' ) ?></a>
							</div>
						<?php } ?>
					</div>
				</div>
...
class RoslynElatedRepeater implements iRoslynElatedRender {
	private $label;
	private $description;
	private $name;
	private $fields;
			...
			<?php if ($this->table_layout) { ?>
				<div class="eltdf-repeater-table-heading">
					<div class="eltdf-repeater-fields-holder">
						<div class="eltdf-repeater-table-cell eltdf-repeater-sort"><?php esc_html_e( 'Order', 'roslyn' ) ?></div>
						...
						<div class="eltdf-repeater-table-cell eltdf-repeater-remove"><?php esc_html_e( 'Remove', 'roslyn' ) ?></div>
					</div>
				</div>
			<?php } ?>
																			...
																			<div class="eltdf-repeater-inner-fields-row <?php echo esc_attr( $col_width_inner_class ); ?>">
																				<div class="eltdf-repeater-inner-fields-row-inner">
																					...
																					if ( ! empty( $field_inner['dependency'] ) ) {
																						$dependencyValues = roslyn_elated_return_repeater_dependency_options_array(array(
																							'field' 	   => $field,
...
class RoslynElatedFieldAddress extends RoslynElatedFieldType {
				...
				<div class="container-fluid">
					<div class="row">
						...
						<?php endif; ?>
							...
							<button id="find" class="btn btn-primary"><?php esc_html_e( 'Place the pin on the map', 'roslyn' ); ?></button>
...
class RoslynElatedFieldIcon extends RoslynElatedFieldType {
    public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $repeat = array() ) {
        $class = '';
        ...
        $icon_packs        = roslyn_elated_icon_collections()->getIconCollectionsEmpty();
