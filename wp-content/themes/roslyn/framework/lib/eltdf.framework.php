...
class RoslynElatedFramework {
	...
	private function __construct() {
		$this->eltdOptions         = RoslynElatedOptions::get_instance();
		$this->eltdMetaBoxes       = RoslynElatedMetaBoxes::get_instance();
		$this->eltdTaxonomyOptions = RoslynElatedTaxonomyOptions::get_instance();
		$this->eltdUserOptions     = RoslynElatedUserOptions::get_instance();
		$this->eltdDashboardOptions = RoslynElatedDashboardOptions::get_instance();
	}
	...
	public function setSkin( RoslynElatedSkinAbstract $skinObject ) {
		$this->skin = $skinObject;
	}
}

/**
 * Class RoslynElatedSkinManager
 *
...
class RoslynElatedSkinManager {
	...
	/**
	 * @see RoslynElatedSkinManager::setSkin()
	 */
	...
	private function setSkin( $skinName = ELATED_PROFILE_SLUG ) {
		if ( $skinName !== '' ) {
			if ( file_exists( get_template_directory() . '/framework/admin/skins/' . $skinName . '/skin.php' ) ) {
				...
				$skinName = ucfirst( $skinName ) . esc_html__( 'Skin', 'roslyn' );
...
/**
 * Class RoslynElatedSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class RoslynElatedSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	public function loadTemplatePart( $template, $params = array() ) {
		...
		if ( $template !== '' ) {
			include( roslyn_elated_find_template_path( 'framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php' ) );
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.eltdSCIcon = '<?php echo roslyn_elated_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
			window.eltdSCLabel = '<?php echo esc_html( ucfirst( $this->skinName ) ); ?> <?php esc_html_e( 'Shortcodes', 'roslyn' ) ?>';
		</script>
	<?php }
...
class RoslynElatedOptions {
	...
	public function getOptionValue( $key ) {
		...
		if ( array_key_exists( $key, $roslyn_elated_options ) ) {
			return $roslyn_elated_options[ $key ];
...
/*
   Class: RoslynElatedAdminPage
   A class that initializes Elated Admin Page
*/
class RoslynElatedAdminPage implements iRoslynElatedLayoutNode {
	public $layout;
	private $factory;
	public $slug;
	public $title;
	...
	function __construct( $slug = "", $title = "", $icon = "" ) {
		$this->layout  = array();
		$this->factory = new RoslynElatedFieldFactory();
		$this->slug    = $slug;
		$this->title   = $title;
		$this->icon    = $icon;
	}
	...
	public function renderChild( iRoslynElatedRender $child ) {
		$child->render( $this->factory );
	}
}

/*
   Class: RoslynElatedMetaBoxes
...
/*
   Class: RoslynElatedMetaBox
   A class that initializes Elated Meta Box
*/
class RoslynElatedMetaBox implements iRoslynElatedLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	public $title;
	...
	public function setTitle( $label ) {
		...
		return $roslyn_elated_Framework->getSkin()->getSkinLabel() . ' ' . $label;
	}
}

/*
   Class: RoslynElatedTaxonomyOptions
   A class that initializes RoslynElated Taxonomy Options
*/
class RoslynElatedTaxonomyOptions {
	private static $instance;
...
/*
   Class: RoslynElatedTaxonomyOption
   A class that initializes RoslynElated Taxonomy Option
*/
class RoslynElatedTaxonomyOption implements iRoslynElatedLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new RoslynElatedTaxonomyFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: RoslynElatedUserOptions
   A class that initializes RoslynElated User Options
*/
class RoslynElatedUserOptions {
	private static $instance;
	public $userOptions;
...
/*
   Class: RoslynElatedUserOption
   A class that initializes RoslynElated User Option
*/
class RoslynElatedUserOption implements iRoslynElatedLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new RoslynElatedUserFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: RoslynElatedDashboardOptions
   A class that initializes RoslynElated Dashboard Options
*/
class RoslynElatedDashboardOptions {
	private static $instance;
...
/*
   Class: RoslynElatedDashboardOption
   A class that initializes RoslynElated Dashboard Option
*/
class RoslynElatedDashboardOption implements iRoslynElatedLayoutNode {
	public $layout;
	private $factory;
	
	function __construct() {
		$this->layout        = array();
		$this->factory       = new RoslynElatedDashboardFieldFactory();
	}
...
if ( ! function_exists( 'roslyn_elated_init_framework_variable' ) ) {
	function roslyn_elated_init_framework_variable() {
		global $roslyn_elated_Framework;
		
		$roslyn_elated_Framework = RoslynElatedFramework::get_instance();
		$eltdSkinManager                 = new RoslynElatedSkinManager();
		$roslyn_elated_Framework->setSkin( $eltdSkinManager->getSkin() );
	}
	
	add_action( 'roslyn_elated_before_options_map', 'roslyn_elated_init_framework_variable' );
}
?>