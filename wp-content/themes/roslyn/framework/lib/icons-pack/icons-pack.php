...
if ( ! function_exists( 'roslyn_elated_get_icon_pack_collections' ) ) {
	function roslyn_elated_get_icon_pack_collections() {
		$options = apply_filters( 'roslyn_elated_add_icon_pack_into_collection', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'roslyn_elated_get_icon_pack_collections' );
}

if ( ! function_exists( 'roslyn_elated_get_icon_pack_options' ) ) {
	function roslyn_elated_get_icon_pack_options() {
		$options = apply_filters( 'roslyn_elated_add_icon_pack_into_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'roslyn_elated_get_icon_pack_options' );
}

if ( ! function_exists( 'roslyn_elated_get_icon_pack_social_options' ) ) {
	function roslyn_elated_get_icon_pack_social_options() {
		$options = apply_filters( 'roslyn_elated_add_icon_pack_into_social_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'roslyn_elated_get_icon_pack_social_options' );
}

/*
 * Class: RoslynElatedIconCollections
 */
class RoslynElatedIconCollections {
	...
	private function __construct() {
		...
		$this->iconPackCollections   = roslyn_elated_get_icon_pack_collections( true );
		$this->iconPackOptions       = roslyn_elated_get_icon_pack_options();
		$this->iconPackSocialOptions = roslyn_elated_get_icon_pack_social_options();
	...
	public function getIconsMetaBoxOrOption( $attributes ) {
		...
		foreach ( $icon_collections as $collection_key => $collection_object ) {
			...
			$icon_pack_container = roslyn_elated_add_admin_container(
				array(
					'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'roslyn' ),
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'        => 'dropdown',
					'heading'     => esc_html__( 'Icon', 'roslyn' ),
	...
	public function getIconWidgetParamsArray() {
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'roslyn' ),
			'options' => $this->getIconPackOptions()
		);
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'roslyn' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'roslyn' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'roslyn' ), $n )
			);
			
			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'roslyn' ), $n ),
	...
	public function getIconSizesArray() {
		return array(
			esc_html__( 'Tiny', 'roslyn' )       => 'fa-lg',
			esc_html__( 'Small', 'roslyn' )      => 'fa-2x',
			esc_html__( 'Medium', 'roslyn' )     => 'fa-3x',
			esc_html__( 'Large', 'roslyn' )      => 'fa-4x',
			esc_html__( 'Very Large', 'roslyn' ) => 'fa-5x'
		);
	}
	...
	public function getIconCollectionIcons( iRoslynElatedIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'roslyn' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                           = array();
		$vc_array[ esc_html__( 'No Icon', 'roslyn' ) ] = '';
